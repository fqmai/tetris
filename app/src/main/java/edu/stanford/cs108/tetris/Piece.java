...
public class Piece {
	...
	private int width;
	private int height;
	private Piece next = null; // "next" rotation
	private int length;
	...
	public Piece(TPoint[] points) {
		...
		int[] checkEmptyX = new int[points.length];
		int[] checkEmptyY = new int[points.length];
		// initialize length
		...
		for (int i = 0; i < length; i++) {
			if (points[i].x < 0 || points[i].y < 0) {
				throw new RuntimeException("Buggy piece: negative TPoints");
			}
			body[i] = new TPoint(points[i]);
			checkEmptyX[i] = points[i].x;
			checkEmptyY[i] = points[i].y;
			max_x = Math.max(points[i].x, max_x);
			max_y = Math.max(points[i].y, max_y);
		}
		Arrays.sort(checkEmptyX);
		Arrays.sort(checkEmptyY);
		for (int i = 0; i < points.length - 1; i++) {
			if (checkEmptyX[i+1] - checkEmptyX[i] > 1 ||checkEmptyY[i] - checkEmptyY[i+1] > 1) {
				throw new RuntimeException("Buggy piece: empty rows or columns");
			}
		}
		...
		for (int j = 0; j < width; j++) {
			skirt[j] = Integer.MAX_VALUE;
		}
		for (int i = 0; i < length; i++) {
			for (int j = 0; j < width; j++) {
				if (points[i].x == j) {
					skirt[j] = Math.min(points[i].y, skirt[j]);
				}
			}
		}
	}
	...
	public Piece computeNextRotation() {
		TPoint[] newBody = new TPoint[length];
		for (int i = 0; i < length; i++) {
			TPoint newPoint = new TPoint(height - body[i].y - 1, body[i].x);
			newBody[i] = newPoint;
		}
		Piece next = new Piece(newBody);
		return next;
	}
	...
	public boolean equals(Object obj) {
		...
		if(other.body.length != this.length) return false;
		boolean[][] mat = new boolean[this.length][this.length];
	...
	*/
	...
	/**
	 ...
	 (Provided code)
	 */
	public static TPoint[] parsePoints(String string) {
